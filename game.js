...
function renderCurrentLetter() {
  ...
  document.title = `${item.upper} - Sò học chữ cái`;
...
function createQuestion() {
  ...
  questionAnswered = false;
  answerBusy = false;
  playFeedback.textContent = "Sò hãy chọn hình đúng nhé!";
...
async function checkPlayAnswer(button, selectedItem) {
  ...
  if (!isCorrect) {
    ...
    playFeedback.textContent = `${selectedItem.upper} chưa đúng. Sò thử lại nhé!`;
  ...
  playFeedback.textContent = `Sò chọn đúng rồi! ${currentCorrectItem.upper} như ${currentCorrectItem.word.trim()}.`;
...
function playQuestionSound({ automatic = false } = {}) {
  ...
  playAudio(getAudio(currentCorrectItem.sound), sequenceId).then((ok) => {
    if (ok && !questionAnswered) {
      ...
      // Safari có thể chặn tự phát trong một số trường hợp; nút Sò nghe vẫn dùng được.
      playFeedback.textContent = "Bấm Sò nghe câu hỏi nếu chưa nghe rõ nhé!";
    }
  });
}
...
function showPlayResult() {
  ...
  playInstructionElement.textContent = "Sò đã hoàn thành thử thách!";
...
function getResultMessage(score) {
  if (score === TOTAL_QUESTIONS) return "Xuất sắc! Sò đã trả lời đúng tất cả câu hỏi.";
  if (score >= 8) return "Rất tốt! Sò đã nhớ được nhiều chữ cái.";
  if (score >= 5) return "Khá tốt! Sò hãy luyện thêm một lượt nữa nhé.";
